# Auxiliary functions for Notch-AR project
...
#Modified for AR project by Souvik
...
def parameters(onecell=False):
    dic = {'km': 5.0e-1
        ...
        , 'ldf': 3, 'ljf': 0.3
        , 'It': 0.0
        ,'gA': 10,'A0W':90000,'A0Z':90000,'A0S':90000,'S0A':4.5e+5,'lAW':2,'lAZ':0.5,'lAS':0.5,'Z0A':6.0e+5,'lZA':0.5,'lSA':2,'nAW':2
           }
    if onecell:
...
def equations(onecell=False,andro=False):
    if andro:
        if onecell:
            return {
             'W': 'gu200*HS(AR,A0W,nA,lAW)*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(AR,150000,2,0.5)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) -ku200*W'
            ,'Y': 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(AR,150000,2,0.5)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,km,2) - gN*HS(I,I0,p,ln)*Py(Y,km,2)-gA*HS(S,S0A,2,lSA)*HS(Z,Z0A,2,lZA)*Py(Y,km,2) - gD*HS(I,I0,p,ld)*Py(Y,km,3) - ku34*Y'
            ,'Z': 'kP*gZ*HS(AR,A0Z,nA,lAZ)*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,km,6)- kZ*Z'
            ,'S': 'kP*gS*HS(AR,A0S,nS,lAS)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,km,2)- kS*S'
            ...
            ,'J': 'kP*gJ*HS(I,I0,p,lj)*Pl(W,km,5) - J*(  kc*N*HS(I,I0,pf,) + kt*Nt ) - k*J'
            ,'I': 'kt*N*( Dt*HS(I,I0,pf,ldf) + Jt*HS(I,I0,pf,ljf) ) - kI*I'
            ,'AR': 'kP*gA*HS(Z,Z0A,nA,lZA)*HS(S,S0A,nA,lSA)*Pl(Y,km,2)-k*AR'
            }
        else:
            return {
             'W': 'gu200*HS(AR,A0W,2,lAW)*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(AR,90000,2,0.5)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) -ku200*W'
            ,'Y': 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(AR,90000,2,0.5)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,2,km,u034) - gN*HS(I,I0,p,ln)*Py(Y,2,km,u034)-gA*HS(S,450000,2,2)*HS(Z,600000,2,0.5)*Py(Y,2,km,u034) - gD*HS(I,I0,p,ld)*Py(Y,3,km,u034) - ku34*Y'
            ,'Z': 'kP*gZ*HS(AR,A0Z,2,lAZ)*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,6,km,u0200)- kZ*Z'
            ,'S': 'kP*gS*HS(AR,A0S,2,lAS)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,2,km,u034)- kS*S'
            ...
            ,'AR': 'kP*gA*HS(Z,Z0A,nZA,lZA)*HS(S,S0A,nSA,lSA)*Pl(Y,2,km,u034)-k*AR'
            }
    else:
        if onecell:
            return {
             'W': 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) -ku200*W'
...
def vlim(c=None, andro=False):
    if andro:
        if c == 'E':
            return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.002e+4],'AR': [8.0e+3, 1.0e+4]
            , 'W': [1.5e+4, 3.5e+4], 'Z': [0.0e+5, 0.1e+4], 'Y': [1.6e+4, 2.2e+4], 'S': [0.5e+5, 2.0e+5]
                }
        elif c == 'E/M':
            return {'N': [3.0e+4, 4.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [0.1e+3, 1.0e+3], 'I': [0.5e+3, 1.5e+3],'AR': [8.0e+3, 1.0e+4]
            , 'W': [0.5e+4, 1.5e+4], 'Z': [1.0e+4, 1.5e+5], 'Y': [1.5e+4, 1.7e+4], 'S': [1.7e+5, 2.1e+5]
                }
        elif c == 'M':
            return {'N': [2.0e+4, 3.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [2.0e+3, 5.0e+3], 'I': [0.4e+3, 1.6e+3],'AR': [8.0e+3, 1.0e+4]
            , 'W': [0.0e+4, 0.5e+4], 'Z': [4.0e+5, 8.5e+5], 'Y': [0.6e+4, 1.5e+4], 'S': [2.0e+5, 3.0e+5]
                }
        else:
            return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.2e+4],'AR': [8.0e+3, 1.0e+4]
...
# -------------------------------------------------------------------------------------------------------------------#
def plot_continuation(ODE, freepar, keys, bif_startpoint, LocBifPoints=['LP', 'B'], PCargs=None, returnLP=None,
                      ics=None,
