# Auxiliary functions for the Notch-EMT circuit with Androgen receptor
...
def parameters(onecell=False):
    dic = {'km': 5.0e-1
        ...
        , 'ldf': 3, 'ljf': 0.3
        , 'It': 0.0, 'gA': 10
        ,'A0W':9.0e+4,'A0Z':9.0e+4,'A0S':9.0e+4
        ,'S0A':4.5e+5,'lAW':2,'lAZ':0.5
        ,'lAS':0.5,'Z0A':6.0e+5,'lZA':0.5,'lSA':2,'nA':2
           }
    if onecell:
...
def equations(onecell=False,andro=False):
    if andro:
        if onecell:
            return {
             'W': 'gu200*HS(AR,A0W,nA,lAW)*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(AR,90000,2,0.5)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6)- gJ*HS(I,I0,p,lj)*Py(W,km,5)-ku200*W'
            ,'Y': 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(AR,90000,2,0.5)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,km,2) - gN*HS(I,I0,p,ln)*Py(Y,km,2)-gA*HS(S,S0A,nA,lSA)*HS(Z,Z0A,nA,lZA)*Py(Y,km,2) - gD*HS(I,I0,p,ld)*Py(Y,km,3) - ku34*Y'
            ,'Z': 'kP*gZ*HS(AR,A0Z,nA,lAZ)*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,km,6)- kZ*Z'
            ,'S': 'kP*gS*HS(AR,A0S,nA,lAS)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,km,2)- kS*S'
            ...
            ,'AR': 'kP*gA*HS(S,S0A,nA,lSA)*HS(Z,Z0A,nA,lZA)*Pl(Y,km,2)-k*AR'
            }
        else:
            return {
             'W': 'gu200*HS(AR,A0W,nA,lAW)*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(AR,90000,2,0.5)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) -ku200*W'
            ,'Y': 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(AR,90000,2,0.5)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,2,km,u034) - gN*HS(I,I0,p,ln)*Py(Y,2,km,u034)-gA*HS(S,S0A,nA,lSA)*HS(Z,Z0A,nA,lZA)*Py(Y,2,km,u034) - gD*HS(I,I0,p,ld)*Py(Y,3,km,u034) - ku34*Y'
            ,'Z': 'kP*gZ*HS(AR,A0Z,nA,lAZ)*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,6,km,u0200)- kZ*Z'
            ,'S': 'kP*gS*HS(AR,A0S,nA,lAS)*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,2,km,u034)- kS*S'
            ...
            ,'AR': 'kP*gA*HS(Z,Z0A,nA,lZA)*HS(S,S0A,nA,lSA)*Pl(Y,2,km,u034)-k*AR'
            }
    else:
        if onecell:
            return {
            'W': 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) - gJ*HS(I,I0,p,lj)*Py(W,km,5) - ku200*W'
            ...
            return {
             'W': 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) -ku200*W'
...
def functions():
    return {'HS': (['X', 'X0', 'nX', 'lamb'], 'lamb + (1.0-lamb)/(1.0 + (X/X0)**nX)')
        ...
        , 'Py': (['X', 'kd', 'n'],
                 'sum(i, 0, 6, if([i]>n, 0, gu[i]*C([i],n)*M(X,u0200,[i],n)))/(sum(i, 0, 6, if([i]>n, 0, gm[i]*C([i],n)*M(X,u0200,[i],n))) + kd)')
        , 'Pl': (['X', 'kd', 'n'],
                 'sum(i, 0, 6, if([i]>n, 0,  l[i]*C([i],n)*M(X,u0200,[i],n)))/(sum(i, 0, 6, if([i]>n, 0, gm[i]*C([i],n)*M(X,u0200,[i],n))) + kd)')
            }


# variable limits
def vlim(c=None, andro=False):
    if andro:
        if c == 'E':
            return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.002e+4],'AR': [8000, 10000]
            , 'W': [1.5e+4, 3.5e+4], 'Z': [0.0e+5, 0.1e+4], 'Y': [1.6e+4, 2.2e+4], 'S': [0.5e+5, 2.0e+5]
                }
        elif c == 'E/M':
            return {'N': [3.0e+4, 4.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [0.1e+3, 1.0e+3], 'I': [0.5e+3, 1.5e+3],'AR': [8000, 10000]
            , 'W': [0.5e+4, 1.5e+4], 'Z': [1.0e+4, 1.5e+5], 'Y': [1.5e+4, 1.7e+4], 'S': [1.7e+5, 2.1e+5]
                }
        elif c == 'M':
            return {'N': [2.0e+4, 3.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [2.0e+3, 5.0e+3], 'I': [0.4e+3, 1.6e+3],'AR': [8000, 10000]
            , 'W': [0.0e+4, 0.5e+4], 'Z': [4.0e+5, 8.5e+5], 'Y': [0.6e+4, 1.5e+4], 'S': [2.0e+5, 3.0e+5]
                }
        else:
            return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.2e+4],'AR': [8000, 10000]
...
def euler_traj(eqs, p, pts=None, vlim=None, hexagonal=True,
    ...
    if pts == None:
        if vlim == None:
            print
            'ERROR: Give me a starting point (pts) or the limits for a random start point (vlim)'
            return 0
        pts = {}
...
def plot_hex(M, clim=None, cmap=None, clabel=None, fig_name=None, title=None, tr=None, c=None,
    ...
    fig, ax = plt.subplots(figsize=(11.6, 10.0)) #11.6, 10.0))
    if cmap == None:
        cmap = plt.cm.Spectral_r

    if tr == None:
        im = ax.scatter(x, y, c=m, s=700000 / (n * n),edgecolors='black', cmap=cmap, linewidths=1, marker=(6, 0, 0))#line width 1
    else:
        t = {}
        ...
        for i in np.arange(len(tr), 0, -1):
            m[t[i - 1]] = c[i - 1]
        im = ax.scatter(x, y, c=m, s=700000 / (n * n), cmap=cmap,edgecolors='black', linewidths=1, marker=(6, 0, 0))#i CHANGED THICKNESS AND COLOUR OF BORDER OF CELLS
    ...
    if fig_name != None:
        ...
        #plt.savefig(fig_name, format='png', dpi=dpi)
    plt.show()
...
def plot_fractionStates(eqs, p, v, r_v, key, tr, pts_i=None, vlim=None, l=['M', 'E/M', 'E'],
                        c=['#e8656c', '#e4fc36', '#00ff9c'],
                        show_snapshot=False, nsignal_dict={'N': ['D', 'J'], 'I': ['D', 'J'], 'D': ['N'], 'J': ['N']},
                        fig_name=None):
    fs = []
    for i in range(len(r_v)):
        if v == 'fng':
...
# -------------------------------------------------------------------------------------------------------------------#
def plot_fates(dic, keys, tr, colors=['#e8656c', '#e4fc36', '#00ff9c'], vlim=[0.0, 1.0], ncomb=1, fontsize=16,
               fig_name=None):
...
# -------------------------------------------------------------------------------------------------------------------#
def plot_continuation(ODE, freepar, keys, bif_startpoint, LocBifPoints=['LP', 'B'], PCargs=None, returnLP=['LP', 'B'],
                      ics=None,
                      xlim=None, ylim=None, xticks=False, yticks=False, maxstep=1e+2, minstep=1e-2, step=5e+1,
                      maxpoints=10000,
                      off_points=True, nrow=None, ncol=None, showcurve=True, n_form_coef=False, silence=False,
                      fs=[3, 2], fontsize=8, fig_name=False):
    ...
    for j in range(len(ics)):
        ...
        if showcurve:
            for i in range(len(keys)):
                PyCont.display((freepar, keys[i]), stability=True, axes=(nrow, ncol, i + 1), color='k', linewidth=1)
                if off_points:
                    PyCont.plot.toggleLabels('off')
                plt.xlabel(freepar, fontsize=10)
                plt.ylabel(keys[i], fontsize=10)#18
                plt.title('')
                if xlim != None:
    ...
    if returnLP is not None:
        for k in returnLP:  # Loop over requested bifurcation points
            i = 1
            while True:
                sp=PyCont[ODE.name].getSpecialPoint(f"{k}{i}")
                if not sp:
                    break
                x_val=sp[freepar]
                y_val=",".join([f"{key}={sp[key]}" for key in keys if key in sp])
                print(f"{k}{i} occurs at{freepar}={x_val};{y_val}")
                i+=1
...
# -------------------------------------------------------------------------------------------------------------------#
def plot_continuation_twocells(ODE, freepar, keys, bif_startpoint, LocBifPoints=['LP', 'BP', 'B'], xlim=False,
                               ylim=False,
                               maxpoints=500, fig_name=False, saveeigen=True, maxstep=1e-0, minstep=1e-2, step=5e-2,
                               silence=False):
    if silence:
        class NullDevice():
            def write(self, s):
    ...
    for i in range(len(l) - 1):
        ...
        fp = eliminate_redundants(pp.find_fixedpoints(ODE, n=2, maxsearch=1e+4, eps=1e-10), 6)
        print(len(fp), fp)
        PCargs = PyCont_args(ODE.name, freepar, maxpoints, stopAt=['B', 'BP'], maxstep=maxstep, minstep=minstep,
                             step=minstep)
...
# -------------------------------------------------------------------------------------------------------------------#
def plot_phasediagram(ODE, freepar, v, r_v, bif_startpoint, keys=False, xlim=False, ylim=False, xticks=False,
                      yticks=False,
                      show_continuation=False, maxstep=1e+2, minstep=1e-2, step=5e+1, maxpoints=500, nrow=None,
                      ncol=None,
                      LocBifPoints=['LP', 'B'], BifPoints=['LP'], silence=False, fig_name=False, fast_fp=False,
                      returnLPs=False):
    if silence:
        class NullDevice():
            def write(self, s):
    ...
    for i in r_v:
        ...
        ODE.set(ics=fp)
        PCargs = PyCont_args(ODE.name, freepar, maxpoints, saveeigen=True, maxstep=maxstep, minstep=minstep, step=step,
                             LocBifPoints=LocBifPoints)
        lp = plot_continuation(ODE, freepar, keys, bif_startpoint, PCargs=PCargs, returnLP=BifPoints,
    ...
    plt.xlabel(freepar, fontsize=10)#WAS 18
...
def nullclines(axis, DSargs, stab, fp, nfp=0, vlim=None, c=['b', 'g'], maxpoints=[1000, 1000], step=5e+1, minstep=1e-1,
               maxstep=1e+3, fs=[4, 3], fig_name=False, plotaxis=[0, 1], loc=0, fontsize=18, pcontour=None,
               silence=False,
               xticks=False, yticks=False):
    plt.rcParams.update({'font.size': fontsize}) #FS =[6,5]
    if silence:
        class NullDevice():
            def write(self, s):
                pass
    ...
    for i in plotaxis:
        ...
        PCargs = PyCont_args('nullclines', axis[i], maxpoints[i], maxstep=maxstep, minstep=minstep, step=step,
                             LocBifPoints=['B'])
        ...
        PyCont.display((axis[0], axis[1]), stability=True, linewidth=3, color=c[i],
                       label='d' + axis[i] + '/dt' + ' != 0')
        PyCont.plot.toggleLabels('off')
    ...
    # Generate a grid of points for vector field
    x_vals = np.linspace(vlim[axis[0]][0], vlim[axis[0]][1], 20)
    y_vals = np.linspace(vlim[axis[1]][0], vlim[axis[1]][1], 20)
...
def param_sensitivity_bars(list_pars, ODE, DSargs, var, fig_name=False, fs=[10, 5], delta=[0.0, 0.1, -0.1]):
    ...
    figure(figsize=(fs[0], fs[1]), dpi=200)
    plt.bar(range(len(change.keys())), [change[l[i]][0] for i in isort], color='r', align='center', alpha=0.8,edgecolor='black',  # Set border color
    linewidth=1.6)
    plt.bar(range(len(change.keys())), [change[l[i]][1] for i in isort], color='b', align='center', alpha=0.8,edgecolor='black',  # Set border color
    linewidth=1.6)
    plt.xticks(np.arange(len(isort)), [l[i] for i in isort])
...
# -------------------------------------------------------------------------------------------------------------------#
def param_sensitivity_bifurcations(DSargs, freepar, key, list_pars, bif_startpoint, d=[1.0, 2, 4],
                                   c=['k', 'b', 'r'],
    ...
    for i in range(len(list_pars)):
        ...
        for j in range(len(d)):
            ...
            PCargs = PyCont_args('psensit', freepar, 10000, saveeigen=True, maxstep=maxstep, minstep=minstep, step=step,
                                 LocBifPoints=['B', 'LP'])
            PyCont = PyDSTool.ContClass(ODE)
            PyCont.newCurve(PCargs)
